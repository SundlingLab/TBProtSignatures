...
number_publications[which(is.na(number_publications$NewName)),"NewName"] <- number_publications[which(is.na(number_publications$NewName)),"Var1"]
...
### Calculate frequencies of proteins in targeted and non-targeted studies (Figure 2)
...
levels(number_publications$color)=c("Overlap","Targeted","Non-Targeted")

pdf("New Figures/HypotheisDiscovery.pdf",width=30,height = 10)
protein_order=number_publications[order(number_publications$Freq,decreasing = TRUE),"Var1"]
ggplot(number_publications,aes(x=factor(Var1,protein_order),y=Freq,fill=color))+
  geom_col()+
...
pdf("New Figures/HypotheisDiscovery-stacked.pdf",width=30,height = 10)
ggplot(merged_table,aes(x=factor(Symbol,protein_order),y=value,fill=variable))+
  geom_bar(stat = "identity") + 
  scale_fill_manual(values=c("Non-Targeted"="steelblue","Targeted"="#e7b99d"))+
  theme(axis.text.x =element_text(angle = 90,hjust=1,vjust=0.5),axis.ticks.x = element_blank())+
  labs(fill="",x="",y="Number of publications")+
  scale_y_continuous(expand=c(0,0))
dev.off()
...
colors=c("Targeted"="#e7b99d","Non-Targeted"="steelblue","Overlap"="#fe938b")
...
ggplot(d,aes(x=factor(Gene,level=gene_order),y=Conc,fill=Color))+
  ...
  theme(axis.text.x = element_text(hjust=1,vjust=0.5,angle = 90,colour = colors[d$Color]),axis.ticks.x = element_blank(),legend.text = element_text(size=7),axis.title.y = element_text(size=7),axis.text = element_text(size=7))+
...
### Functional enrichment analysis on overlapping proteins to use in the discussion of manuscript
...
result <- enriched[[1]]
result <- result %>% filter(P.value<0.01)
p1 <- plotEnrich(result, showTerms = 50, numChar = 40, y = "Count", orderBy = "P.value",title="GO Biological Process")


enriched <- enrichr(gene_list, pathways)
result <- enriched[[1]]
result <- result %>% filter(P.value<0.01)
p2 <- plotEnrich(result, showTerms = 50, numChar = 40, y = "Count", orderBy = "P.value",title="KEGG Pathway")

enriched <- enrichr(gene_list, databases)
result <- enriched[[1]]
result <- result %>% filter(P.value<0.01)
p3 <- plotEnrich(result, showTerms = 50, numChar = 40, y = "Count", orderBy = "P.value",title="DisGeNET")

enriched <- enrichr(gene_list, TFs)
result <- enriched[[1]]
result <- result %>% filter(P.value<0.01)
p4 <- plotEnrich(result, showTerms = 50, numChar = 40, y = "Count", orderBy = "P.value",title="TRRUST")


pdf("New Figures/FunctionalEnrichmentOverlap.pdf",width=40,height = 10)
